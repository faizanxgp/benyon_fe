            ...
            <>
                ...
                <Menu.Items modal={false} ref={setDropdownContent} style={styles.popper} {...attributes.popper} className="absolute border border-gray-200 dark:border-gray-800 bg-white dark:bg-gray-950 rounded-md shadow z-[1000] min-w-[180px]">
                    <ul className="py-2">
                        ...
                        <li>
                            <Menu.Item as="button" className="w-full relative px-5 py-2.5 flex items-center rounded-[inherit] text-xs leading-5 font-medium text-slate-600 dark:text-slate-300 hover:text-primary-600 hover:bg-slate-50 hover:dark:bg-gray-900 transition-all duration-300">
                                <Icon className="text-start text-lg leading-none w-7 opacity-80" name="user" />
                        ...
                        <li>
                            <Menu.Item as="button" className="w-full relative px-5 py-2.5 flex items-center rounded-[inherit] text-xs leading-5 font-medium text-slate-600 dark:text-slate-300 hover:text-primary-600 hover:bg-slate-50 hover:dark:bg-gray-900 transition-all duration-300">
                                <Icon className="text-start text-lg leading-none w-7 opacity-80" name="check-round" />
                                <span>Approved</span>
                            </Menu.Item>
                        </li>
                        <li>
...
const KYCListPage = () => {
  ...
  return (
    <>
        ...
        <PageHead>
            <PageHead.Group>
                <PageHead.Title>KYC Documents</PageHead.Title>
                <PageHead.SubTitle>You have total <span className='text-slate-600'>1,257</span> KYC documents.</PageHead.SubTitle>
            </PageHead.Group>
            <PageHead.Group>
                <ul className="flex items-center gap-4 px-3.5 py-5 sm:py-0">
                    <li>
                        <Button size="rg" variant="white-outline">
                            ...
                            <span className="ms-3 hidden md:block">Export</span>
                        </Button>
                    </li>
                ...
                <thead>
                    <tr>
                        ...
                        <th className="py-2 px-2 first:ps-6 last:pe-6 border-b border-t border-gray-300 dark:border-gray-900 text-start hidden xs:table-cell">
                            <span className="block text-sm leading-relaxed text-slate-400 font-normal">Doc Type</span>
                        </th>
                        <th className="py-2 px-2 first:ps-6 last:pe-6 border-b border-t border-gray-300 dark:border-gray-900 text-start hidden md:table-cell">
                            <span className="block text-sm leading-relaxed text-slate-400 font-normal">Documents</span>
                        </th>
                        <th className="py-2 px-2 first:ps-6 last:pe-6 border-b border-t border-gray-300 dark:border-gray-900 text-start hidden lg:table-cell">
                            <span className="block text-sm leading-relaxed text-slate-400 font-normal">Submitted</span>
                        </th>
                        <th className="py-2 px-2 first:ps-6 last:pe-6 border-b border-t border-gray-300 dark:border-gray-900 text-start hidden lg:table-cell">
                        ...
                        <th className="py-2 px-2 first:ps-6 last:pe-6 border-b border-t border-gray-300 dark:border-gray-900 text-start hidden 2xl:table-cell">
                            <span className="block text-sm leading-relaxed text-slate-400 font-normal">Checked By</span>
                        </th>
                    ...
                    return(
                        <tr key={index} className="transition-all duration-300 hover:bg-gray-50 hover:dark:bg-gray-1000 group">
                            ...
                            <td className="py-4 px-2 first:ps-6 last:pe-6 border-b border-gray-300 dark:border-gray-900">
                                <Link to={`/kyc-details/${item.id}`} className="flex items-center">
                                    <div className="flex items-center">
                                        {item.avatar.image ? <Avatar size="rg" rounded img={item.avatar.image}/> : <Avatar variant={item.avatar.theme} size="rg" rounded text={toInitials(item.username)}/>}
                                        <div className="ms-4">
                                            <span className="block text-sm font-medium leading-6 text-slate-700 dark:text-white">
                                                {item.username}
                                                {item.status === "approved" && <span className="h-2 w-2 inline-block rounded-full bg-green-600 lg:hidden ms-1"></span>}
                                                {item.status === "pending" && <span className="h-2 w-2 inline-block rounded-full bg-cyan-600 lg:hidden ms-1"></span>}
                                                {item.status === "rejected" && <span className="h-2 w-2 inline-block rounded-full bg-red-600 lg:hidden ms-1"></span>}    
                                            </span>
                                            <span className="block text-xs leading-4 text-slate-400">{item.id}</span>
                                        </div>
                                    </div>
                                </Link>
                            </td>
                            <td className="py-4 px-2 first:ps-6 last:pe-6 border-b border-gray-300 dark:border-gray-900 hidden xs:table-cell">
                                <span className="text-sm font-normal text-slate-600 dark:text-white">{item.docType}</span>
                            </td>
                            <td className="py-4 px-2 first:ps-6 last:pe-6 border-b border-gray-300 dark:border-gray-900 hidden md:table-cell">
                                <ul className="flex gap-4 text-sm font-normal text-slate-400">
                                    {item?.documents?.map((item,index)=>
                                        <li key={index} className="inline-flex items-center align-middle gap-2.5">{item} <span><em className="icon ni ni-download text-primary-500 text-xl"></em></span>
                                        </li>
                                    )}
                            </ul>
                            </td>
                            ...
                            <td className="py-4 px-2 first:ps-6 last:pe-6 border-b border-gray-300 dark:border-gray-900 hidden md:table-cell">
                                {item.status === "approved" && <span className="text-sm font-medium text-green-600">Approved </span>}
                                {item.status === "pending" && <span className="text-sm font-medium text-cyan-600">Pending </span>}
                                {item.status === "rejected" && <span className="text-sm font-medium text-red-600">Rejected </span>}
                                {
                                    (item.status == 'approved' || item.status == 'rejected' ) && (
                                        <Tooltip placement="top" className="text-sm" content={`${item.status.charAt(0).toUpperCase() + item.status.slice(1)} at ${item.submitted}`}>
                                           <Icon name="info" />
                                        </Tooltip>
                                    )
                                }
                            ...
                            <td className="py-4 px-2 first:ps-6 last:pe-6 border-b border-gray-300 dark:border-gray-900 hidden 2xl:table-cell">
                                <div className="flex items-center gap-3">
                                    <div className="relative flex-shrink-0 flex items-center justify-center text-sm bg-orange-100 dark:bg-orange-950 text-orange-600 h-10 w-10 rounded-full font-medium">
                                        <Avatar size="rg" rounded text={toInitials(item?.checkedBy.name)}/>
                                    </div>
                                    <span className=" text-slate-700 dark:text-white text-xs font-medium">{item.checkedBy.name}</span>
                                </div>
                            </td>
                                ...
                                <ul className="relative flex items-center justify-end -me-2">
                                    <li className="opacity-0 transition-all duration-300 bg-gray-50 dark:bg-gray-1000 px-0.5 group-hover:opacity-100">
                                        <Tooltip placement="top" content="Wallet">
                                            <Button.Zoom>
                                                <Icon className="text-xl/4.5" name="eye-fill" />
                                            </Button.Zoom>
                                        </Tooltip>
                                    </li>
                                    <li className="opacity-0 transition-all duration-300 bg-gray-50 dark:bg-gray-1000 px-0.5 group-hover:opacity-100">
                                        <Tooltip placement="top" content="Send Email">
                                            <Button.Zoom>
                                                <Icon className="text-xl/4.5" name="check-fill-c" />
                                            </Button.Zoom>
                                        </Tooltip>
                                    </li>
                                    <li className="opacity-0 transition-all duration-300 bg-gray-50 dark:bg-gray-1000 px-0.5 group-hover:opacity-100">
                                        <Tooltip placement="top" content="Suspend">
                                            <Button.Zoom>
                                                <Icon className="text-xl/4.5" name="cross-fill-c" />
                                            </Button.Zoom>
                                        </Tooltip>
                                    </li>
                                    <li>
