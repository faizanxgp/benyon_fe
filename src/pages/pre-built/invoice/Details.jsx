...
const InvoiceDetailsPage = () => {
  ...
  return (
    <>
        <Head title={`Invoice ${data.orderId}`} />
        <PageHead>
            <PageHead.Group>
                <PageHead.Title>Invoice <strong className="text-primary-600 text-2xl font-normal">#{data.orderId}</strong></PageHead.Title>
                <PageHead.SubTitle>
                    Created At:  <span className="text-slate-600 dark:text-slate-400">{data.date}, {data.time}</span>
                </PageHead.SubTitle>
            </PageHead.Group>
            <PageHead.Group>
                <Button as="Link" size="rg" variant="white-outline" className="hidden sm:inline-flex" to="/invoice-list">
                    <Icon className="text-xl leading-4.5 rtl:-scale-x-100" name="arrow-left" />
                    <span className="ms-3">Back</span>
                </Button>
                <Button as="Link" size="rg" variant="white-outline" className="sm:hidden" icon to="/invoice-list">
                    <Icon className="text-xl leading-4.5 rtl:-scale-x-100" name="arrow-left" />
                </Button>
            </PageHead.Group>
        </PageHead>
                ...
                <div className="flex flex-col md:flex-row justify-between gap-6 pb-6">
                    <div className="relative">
                        ...
                        <div className="invoice-contact-info">
                            <h4 className="text-2xl text-slate-700 dark:text-white font-heading font-bold tracking-tight mb-4">{data.name}</h4>
