...
const Radio = React.forwardRef(({id, checked, disabled, name, size, className, labelClass, required,children, ...rest},ref) => {
  ...
  return (
    <div className="inline-flex">
        <input type="radio" ref={ref} className={inputClass} defaultChecked={checked && checked} disabled={disabled && disabled}  name={name && name}  id={id} required={required && "required"} {...rest} />
...
const RadioImage = ({id, checked, disabled, name, size, className, labelClass, src, alt, required, ...props}) => {
  ...
  return (
    <div className="relative z-[1] inline-flex group">
        <input type="radio" className={inputClass} defaultChecked={checked && checked} disabled={disabled && disabled}  name={name && name}  id={id} required={required && "required"} />
...
const RadioButton = React.forwardRef(({id, checked, disabled, name, size, className, labelClass, iconClass, nocontrol, icon, required,children, ...rest}, ref) => {
  const inputClass = classNames({
    "peer absolute" : true,
    "start-4 top-1/2 -translate-y-1/2 bg-white dark:bg-gray-950 checked:bg-primary-600 checked:dark:bg-primary-600 checked:hover:bg-primary-600 checked:hover:dark:bg-primary-600 checked:focus:bg-primary-600 checked:focus:dark:bg-primary-600 focus:border-primary-600 focus:dark:border-primary-600 outline-none focus:outline-offset-0 focus:outline-primary-200 focus:dark:outline-primary-950 focus:ring-0 focus:ring-offset-0 disabled:bg-slate-50 disabled:dark:bg-slate-900 disabled:checked:bg-primary-400 disabled:checked:dark:bg-primary-400 rounded-full border-2 border-gray-300 dark:border-gray-900  cursor-pointer disabled:cursor-not-allowed transition-all duration-300" : !nocontrol,
...
const RadioColor = React.forwardRef(({id, checked, disabled, name, size, className, required, children, ...rest}, ref) => {
  const inputClass = classNames({
    "relative rounded-full cursor-pointer transition-all duration-300 border-0 checked:bg-none checked:dark:shadow-none checked:ring-2 ring-offset-2 ring-primary-600 ring-offset-white dark:ring-offset-gray-950" : true,
    "h-4 w-4" : !size,
    "h-6 w-6" : size === "rg",
    "h-8 w-8" : size === "lg",
    [`${className}`]: className,
  ...
  return (
    <>
        <input ref={ref} type="radio" className={inputClass} defaultChecked={checked && checked} disabled={disabled && disabled}  name={name && name}  id={id} required={required && "required"} {...rest}  />
        {children && <label className={labelClasses} htmlFor={id}>
          {icon && <Icon className={iconClasses} name={icon} />}
          {children}
        </label>}
    </>
